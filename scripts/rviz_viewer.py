...
class RvizViewer:
    def __init__(self):
        ...
        if setting_file_path == "":
            print(
                "Rviz viewer: no setting file path is given, using default setting files --"
                + setting_file_path
            )
        ...
        for i in range(self.robot.num_chain):
            pose_goal_marker = make_marker(
                "arm_" + str(i),
                settings["base_links"][i],
                "widget",
                [0.1, 0.1, 0.1],
                self.ee_poses[i],
                False,
        ...
        for i in range(len(self.robot.articulated_joint_names)):
            self.js_msg.position[
                self.robot.all_joint_names.index(self.robot.articulated_joint_names[i])
            ] = settings["starting_config"][i]
        self.js_msg.header.stamp = rospy.Time.now()
        self.js_pub.publish(self.js_msg)

        rospy.Subscriber(
            "/relaxed_ik/joint_angle_solutions", JointState, self.ja_solution_cb
        )
    ...
    def ja_solution_cb(self, msg):
        ...
        for i in range(len(msg.name)):
            self.js_msg.position[
                self.robot.all_joint_names.index(msg.name[i])
            ] = msg.position[i]
        self.js_pub.publish(self.js_msg)
    ...
    def ee_vel_goal_cb(self, msg):
        ...
        for i in range(self.robot.num_chain):
            ...
            curr_q = [
                self.ee_poses[i].orientation.w,
                self.ee_poses[i].orientation.x,
                self.ee_poses[i].orientation.y,
                self.ee_poses[i].orientation.z,
            ]
            tmp_q = T.quaternion_from_scaledAxis(
                [
                    msg.ee_vels[i].angular.x,
                    msg.ee_vels[i].angular.y,
                    msg.ee_vels[i].angular.z,
                ]
            )
